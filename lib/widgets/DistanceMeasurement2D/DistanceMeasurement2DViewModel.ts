...
export interface DistanceMeasurement2DViewModelProperties {
  view: MapView;
  layer?: GraphicsLayer;
  polylineSymbol?: any;
}
...
function createDefaultPolylineSymbol(): SimpleLineSymbol {
  return new SimpleLineSymbol(new Color([227, 139, 79, 1]), 2, "solid");
}

export default class DistanceMeasurement2DViewModel {
  public view: MapView;
  ...
  private _complete(): void {
    ...
    try {
      const { distance, midpoint } = this._calculateDistance(savedVertices);
      const text = DistanceMeasurement2DViewModel._formatDistance(distance);
    ...
    this.measureGraphic = polylineGraphic;
  ...
  private static _formatDistance(meters: number): string {
    if (meters < 1000) {
  ...
  private _calculateDistance(vertices: number[][]): {
    distance: number;
    midpoint: [number, number];
  } {
    if (vertices.length < 2) {
      return { distance: 0, midpoint: (vertices[0] as [number, number]) || [0, 0] };
    }

    const geojson = arcgisToGeoJSON({ paths: [vertices] });
    const d = length(geojson, { units: "meters" });
    const midPoint = along(geojson, d / 2, { units: "meters" });
    const coords = midPoint.geometry.coordinates as [number, number];

    return { distance: d, midpoint: coords };
  }
  ...
  private _renderMeasurementLabel(ctx: CanvasRenderingContext2D): void {
    ...
    try {
      const { distance, midpoint } = this._calculateDistance(allPoints);
      const text = DistanceMeasurement2DViewModel._formatDistance(distance);
      const [x, y] = this.view.toScreen(midpoint[0], midpoint[1]);
